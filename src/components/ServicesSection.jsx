...
export default function ServicesSection() {
      ...
      visual: (
        <div className="relative w-full min-h-[300px] sm:h-[380px] rounded-2xl bg-gradient-to-br from-[#101014] to-[#050505] border border-white/10 p-3.5 sm:p-6 overflow-hidden flex flex-col justify-between shadow-2xl group">
          ...
          <div className="relative z-10 flex items-center justify-between pb-3 border-b border-white/10">
            ...
            <div className="text-[10px] sm:text-[11px] font-mono text-white/50 px-2.5 sm:px-3 py-0.5 rounded-full bg-white/5 border border-white/5 truncate max-w-[130px] sm:max-w-none">
              dinemotion.studio/web
            </div>
            <div className="w-6 sm:w-8" />
          </div>

          <div className="relative z-10 my-auto flex flex-col gap-2.5 sm:gap-3 py-2">
            <div className="inline-flex items-center gap-1.5 text-xs font-mono text-[#00D6FF]">
              <Sparkles className="w-3 h-3" />
              <span className="text-[11px] sm:text-xs">RESPONSIVE ARCHITECTURE</span>
            </div>
            <h4 className="font-display text-base sm:text-2xl font-semibold text-white tracking-tight leading-snug">
              Editorial Aesthetics Meets Sub-Millisecond Speed.
            </h4>
            <div className="grid grid-cols-3 gap-1 sm:gap-2.5 pt-1 sm:pt-2">
              <div className="p-1.5 sm:p-3 rounded-xl bg-white/[0.02] border border-white/5 text-center sm:text-left min-w-0">
                <span className="block text-sm sm:text-lg font-semibold text-white font-display">100%</span>
                <span className="text-[8px] sm:text-[10px] text-white/50 font-mono truncate block">Performance</span>
              </div>
              <div className="p-1.5 sm:p-3 rounded-xl bg-white/[0.02] border border-white/5 text-center sm:text-left min-w-0">
                <span className="block text-sm sm:text-lg font-semibold text-[#00D6FF] font-display">60 FPS</span>
                <span className="text-[8px] sm:text-[10px] text-white/50 font-mono truncate block">Fluid Motion</span>
              </div>
              <div className="p-1.5 sm:p-3 rounded-xl bg-white/[0.02] border border-white/5 text-center sm:text-left min-w-0">
                <span className="block text-sm sm:text-lg font-semibold text-white font-display">Zero</span>
                <span className="text-[8px] sm:text-[10px] text-white/50 font-mono truncate block">Bloat</span>
              </div>
            </div>
          </div>

          <div className="relative z-10 flex items-center justify-between pt-3 border-t border-white/10 text-[9px] sm:text-[11px] font-mono text-white/50">
      ...
      visual: (
        <div className="relative w-full min-h-[300px] sm:h-[380px] rounded-2xl bg-gradient-to-br from-[#101014] to-[#050505] border border-white/10 p-3.5 sm:p-6 overflow-hidden flex flex-col justify-between shadow-2xl group">
          <div className="relative z-10 flex items-center justify-between pb-3 border-b border-white/10">
            ...
            <span className="text-[10px] sm:text-[11px] font-mono text-[#0050FF]">ACTIVE STATE</span>
          </div>
      ...
      visual: (
        <div className="relative w-full min-h-[300px] sm:h-[380px] rounded-2xl bg-gradient-to-br from-[#101014] to-[#050505] border border-white/10 p-3.5 sm:p-6 overflow-hidden flex flex-col justify-between shadow-2xl group">
          <div className="relative z-10 flex items-center justify-between pb-3 border-b border-white/10">
            ...
            <div className="flex items-center gap-1 text-[10px] sm:text-[11px] font-mono text-red-400">
          ...
          <div className="relative z-10 my-auto py-2">
            <div className="grid grid-cols-2 gap-2 sm:gap-3 mb-3">
              <div className="p-2 sm:p-3 rounded-xl bg-black/40 border border-white/10 min-w-0">
                <span className="text-[9px] sm:text-[10px] font-mono text-white/40 block">RATIO</span>
                <span className="text-[10px] sm:text-sm font-semibold font-mono text-white truncate block">2.39 : 1 ANAMORPHIC</span>
              </div>
              <div className="p-2 sm:p-3 rounded-xl bg-black/40 border border-white/10 min-w-0">
                <span className="text-[9px] sm:text-[10px] font-mono text-white/40 block">GRADING</span>
                <span className="text-[10px] sm:text-sm font-semibold font-mono text-[#00D6FF] truncate block">STUDIO DI</span>
              </div>
            </div>
  ...
  return (
    <section id="services" className="section bg-transparent border-t border-white/[0.08] px-3 sm:px-4">
                  ...
                  <div className={`lg:col-span-7 flex flex-col justify-between ${isReversed ? 'lg:order-2' : 'lg:order-1'}`}>
                    <div>
                      <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-3 sm:mb-5">
                        ...
                        <h3 className="text-[10px] sm:text-xs font-mono font-medium tracking-[0.10em] sm:tracking-[0.14em] uppercase text-white/70 break-words">
                          {service.title}
                        </h3>
                      </div>

                      <h4 className="font-display text-xl sm:text-3xl md:text-4xl font-semibold tracking-tight text-white leading-tight mb-3 sm:mb-4">
                        {service.headline}
                      </h4>

                      <p className="text-xs sm:text-base text-white/65 leading-relaxed mb-5 sm:mb-6 max-w-lg font-light">
                        {service.description}
                      </p>

                      <div className="mb-6 sm:mb-8">
                        <span className="block text-[10px] sm:text-[11px] font-mono uppercase tracking-wider text-white/40 mb-2.5 sm:mb-3">
                        ...
                        <div className="flex flex-wrap gap-1.5 sm:gap-2">
                          {service.tags.map((tag) => (
                            <span
                              key={tag}
                              className="text-[11px] sm:text-xs font-mono text-white/80 px-2.5 sm:px-3 py-0.5 sm:py-1 rounded-full bg-white/[0.03] border border-white/[0.08]"
                            >
                              {tag}
                            </span>
                          ))}
                    ...
                    <div className="pt-4 sm:pt-5 border-t border-white/[0.08] flex flex-wrap items-center justify-between gap-3 text-xs font-mono text-white/50">
